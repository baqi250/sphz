...
define(["dojo/_base/declare", "esri/layers/FeatureLayer","esri/InfoTemplate","esri/layers/ArcGISDynamicMapServiceLayer","esri/layers/ArcGISTiledMapServiceLayer",
        ...
        "esri/symbols/SimpleLineSymbol","esri/symbols/PictureMarkerSymbol","esri/renderers/UniqueValueRenderer","esri/Color","esri/tasks/query", "esri/tasks/QueryTask","esri/tasks/GeometryService","esri/tasks/AreasAndLengthsParameters","dojo/_base/array"],
    function (declare, FeatureLayer,InfoTemplate,ArcGISDynamicMapServiceLayer,ArcGISTiledMapServiceLayer,Point,Polygon,Extent,SpatialReference,Graphic,
    		SimpleFillSymbol,SimpleLineSymbol,PictureMarkerSymbol,UniqueValueRenderer,Color,Query,QueryTask,GeometryService,AreasAndLengthsParams,arrayUtils) {
        var h = declare("StaticLayer", null, {
            ...
            showFeatureLayer: function (featureLayerUrl,layerName) {
                ...
                if(layerName=="xzqj"){
                	//弹出框样式
                     ...
                     var infoContent = '<button class="f-btn" onclick="window.open(\'http://192.168.109.44:8091/${pinyin}/${pinyin}.html\')">显示全景</button>';
                     var title = '<div class="poptitle">地址名称：${name}</div>'
                     infoTemplate.setTitle(title);
                     infoTemplate.setContent(infoContent);
                     featureLayer.setInfoTemplate(infoTemplate);
                     //渲染样式
                	 var pictureMarkerSymbol=new PictureMarkerSymbol("images/poi-b.png",30,30);
                     var uniqueValueRenderer=new UniqueValueRenderer(pictureMarkerSymbol);
            ...
            getFeaturesByGeometry: function (geometry, layerUrl) { 
                ...
                var query = new Query();
                var queryTask = new QueryTask(layerUrl+"/0");
                query.outFields = ['*'];
                ...
                queryTask.execute(query, function (featureSet) {
                	...
                	if(features.length==0){
        				...
        				$("#whmaptoolbar-tool-statics .result-content").append("<ul><li>项目范围与所选图层不相交，无分析结果</li></ul>");
        				$("#whmaptoolbar-tool-statics").show();
                	}else{
                		//求交
                		_this.cutLayer(features, geometry,displayFieldName,function(result){
                			//渲染
                			_this.getLayerSymbol(layerUrl+"/0",function(render,filed){
                    			//显示;
                				for(var i=0;i<result.length;i++){
                					var value=result[i].attributes[filed];
                					//便利render获取样式
                				...
                				_this.groupFeatures(result,filed);
                        	});
                		});	
                	}         	
                });
            ...
            getLayerSymbol:function(layerUrl,callback){
            	$.ajax({
            		...
            		success:function(result){
            			callback(result.drawingInfo.renderer.uniqueValueInfos,result.drawingInfo.renderer.field1);
            		},
            		error:function(e){
            			console.log(e);
            		}
