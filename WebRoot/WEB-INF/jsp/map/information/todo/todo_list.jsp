...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="todo/list.do" method="post" name="Form" id="Form">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:0px;">
						<thead>
							<tr>
								...
								<!-- <th class="center" >待办开始时间</th>
								<th class="center" >待办结束时间</th> -->
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										<td class='center' style="width: 30px;">
											<label><input type='checkbox' name='ids' class="ace" value="${var.todo_id}" /><span class="lbl"></span></label>
										</td>
									...
									<%-- 	<td class="center">${var.begin_time}</td>
										...
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
											...
											<%-- <a style="cursor:pointer;" class="green" onclick="upload('${var.notice_id}');" title="上传附件">
												<i class="ace-icon fa fa-cloud-upload bigger-130"></i>
											</a> --%>
											<c:if test="${QX.edit == 1 }">
											<a style="cursor:pointer;" class="green" onclick="edit('${var.todo_id}');" title="编辑">
												<i class="ace-icon fa fa-pencil bigger-130"></i>
											</a>
											</c:if>
											&nbsp;
											<c:if test="${QX.del == 1 }">
											<a style="cursor:pointer;" class="red" onclick="del('${var.todo_id}');" title="删除">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>todo/goAdd.do';
			 diag.Width = 1000;
			 diag.Height = 800;
		...
		function del(Id){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>todo/delete.do?todo_id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>todo/goEdit.do?todo_id='+Id;
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>todo/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
