...
<!-- <div id="container"> -->
    ...
    <!--         <section class="content-header">
            ...
            <section class="content container-fluid">
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-select">
                            <select id="FWLXID" class="form-control select2" onchange="changeStage()" style="width: 100%;"></select>
                        </div>
				...
				<form role="form" action="sphz/fawen/edit.do" name="editForm" id="editForm" method="post" enctype="multipart/form-data" autocomplete="off">
				...
				<!-- 指标信息（选址、条件（建筑工程））-->
				<div class="box box-success infoDiv unDisDiv" id=infoDiv1>
					<div class="box-header with-border">
								...
								<td>
									<div class="form-group">
										<input type="text" id="GHYDXZDM" name="GHYDXZDM"
											value="${pd.GHYDXZDM }" class="form-control" readonly="readonly">
									</div>
								</td>
							</tr>
							<tr>
    ...
    <script type="text/javascript">
    	...
    	$("#FWLXID_SELECT").val(FWLXID);
   		var stageIndex=0;
   		var html="";
   		if(FWLXID=="选址、条件（建筑工程）"){
   			html+='<option selected="selected" value="选址、条件（建筑工程）">选址、条件（建筑工程）</option>';
   			stageIndex=0;
   			//$("#project_gl_tr").css('display','none');
   		}
   		if(FWLXID=="选址、条件（市政道桥）"){
   			html+='<option selected="selected" value="选址、条件（市政道桥）">选址、条件（市政道桥）</option>';
   			stageIndex=1;
   			//$("#project_gl_tr").css('display','none');
   			$("#XMLX").val("市政道桥").trigger("change");
   		}
   		if(FWLXID=="选址、条件（市政管线）"){
   			html+='<option selected="selected" value="选址、条件（市政管线）">选址、条件（市政管线）</option>';
   			stageIndex=2;
   			//$("#project_gl_tr").css('display','none');
   			$("#XMLX").val("市政管线").trigger("change");
   		}
   		if(FWLXID=="用地许可"){
   			html+='<option selected="selected" value="用地许可">用地许可</option>';
   			stageIndex=3;
   		}
   		if(FWLXID=="建设许可（建筑工程）"){
   			html+='<option selected="selected" value="建设许可（建筑工程）">建设许可（建筑工程）</option>';
   			stageIndex=4;
   		}
   		if(FWLXID=="建设许可（市政道桥）"){
   			html+='<option selected="selected" value="建设许可（市政道桥）">建设许可（市政道桥）</option>';
   			stageIndex=5;
   			$("#XMLX").val("市政道桥").trigger("change");
   		}
   		if(FWLXID=="建设许可（市政管线）"){
   			html+='<option selected="selected" value="建设许可（市政管线）">建设许可（市政管线）</option>';
   			stageIndex=6;
   			$("#XMLX").val("市政管线").trigger("change");
   		}
   		if(FWLXID=="竣工（建筑工程）"){
   			html+='<option selected="selected" value="竣工（建筑工程）">竣工（建筑工程）</option>';
   			stageIndex=7;
   		}
   		if(FWLXID=="竣工（市政道桥）"){
   			html+='<option selected="selected" value="竣工（市政道桥）">竣工（市政道桥）</option>';
   			stageIndex=8;
   			$("#XMLX").val("市政道桥").trigger("change");
   		}
   		if(FWLXID=="竣工（市政管线）"){
   			html+='<option selected="selected" value="竣工（市政管线）">竣工（市政管线）</option>';
   			stageIndex=9;
   			$("#XMLX").val("市政管线").trigger("change");
   		}
   		if(FWLXID=="其它"){
   			html+='<option selected="selected" value="其它">其它</option>';
   			//$("#project_gl_tr").css('display','none');
   			stageIndex=10;
   		}
   		$("#FWLXID").html(html);
		...
		$(".my_select-tree").val('请选择用地性质').trigger("change");
		
		//提醒用户已经有值的文本框
		var inputs=$("input");
		inputs.each(function(){
			if($(this).val()==""){
				$(this).css("background-color","#b5def1");
				$(this).next('span').css("background-color","#b5def1");
			}
		});
		...
		$(function(){ 
	   		var selects= $(".select2-selection__rendered");
			selects.each(function(){
				if($(this).html()==""){
					$(this).parent().css("background-color","#b5def1");
				}
			});
      ...
      //项目红线文件初始化
        ...
        $.ajax({//土地利用类型树状下拉选择框
				...
				success: function(data){
					...
					$(".my_select-tree").select2ToTree({ treeData: { dataArr: dicTreeSelectListData }, maximumSelectionLength: 100, minimumResultsForSearch: -1 });
					$(".my_select-tree").on("select2:select", function() {
					    $("#LAND_USE").val($(".my_select-tree").val());
					});
				
					//规划用地使用性质回显值select-tree2
					var LAND_USE="${pd.LAND_USE}";
					console.log(LAND_USE);
					if(null!=LAND_USE&&LAND_USE!=""&&typeof(LAND_USE)!="undefiend"){
						$(".my_select-tree").val(LAND_USE.split(',')).trigger("change");
					}
					
					var multiSelects= $(".select2-selection--multiple>.select2-selection__rendered");
					multiSelects.each(function(){
						if(!$(this).children().hasClass("select2-selection__choice")){
							$(this).parent().css("background-color","#b5def1");
						}
					});
				},
		...
		//建筑物使用性质回显值
		var JZWSYXZ="${pd.JZWSYXZ}";
		if(null!=JZWSYXZ&&JZWSYXZ!=""&&typeof(JZWSYXZ)!="undefiend"){
			$("#JZWSYXZ").val(JZWSYXZ.split(',')).trigger("change");
		}
		//项目属地回显值
		var XMSD="${pd.XMSD}";
		if(null!=XMSD&&XMSD!=""&&typeof(XMSD)!="undefiend"){
			$("#XMSD").val(XMSD.split(',')).trigger("change");
		}
		...
		//保存按钮点击事件
   		$("#save").on("click",function(){
			...
			$("#save").attr("disabled","disabled");//避免重复提交
   			var newUrl='sphz/fawen/edit.do?FWLXID='+FWLXID;
   			if(FWLXID=="建设许可（建筑工程）"){
