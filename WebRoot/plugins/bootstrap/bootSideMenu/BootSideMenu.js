...
function doAnimation(container, containerWidth, sidebarSide, sidebarStatus){
	...
	if(sidebarStatus=="opened"){
		if(sidebarSide=="left"){
			container.animate({
				left:-(containerWidth+2)+left_width
			});
		...
		if(sidebarSide=="left"){
			container.animate({
				left: $("#right_screen").offset().left + 5
			});
			toggleArrow(toggler, "right");
		}else if(sidebarSide=="right"){
			container.animate({
				right:$("#right_screen").offset().left + 5
			});
			toggleArrow(toggler, "left");
		}
...
function toggleArrow(toggler, side){
	if(side=="left"){
        ...
        $(toggler).css({background:'url('+basePath+'/resources/images/right.png) no-repeat',right:'-20px'});
	}else if(side=="right"){
		$(toggler).children(".glyphicon-chevron-left").css('display', 'none');
		$(toggler).children(".glyphicon-chevron-right").css('display', 'none');
        $(toggler).css({background:'url('+basePath+'/resources/images/left.png) no-repeat'});
        $(toggler).css('right','');
	}
}
