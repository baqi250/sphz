...
public class SphzStatisticController extends BaseController {
	...
	public Object StaticCountByCbbm() throws Exception{
		...
		try {
			...
			document.append("FZRQ", new Document("$gte",DateUtil.getYearFirst(year)).append("$lte", DateUtil.getYearLast(year)));
			...
			String head="";
			if(year==currentYear){
				head+=DateUtil.getYearFirst(year)+"至今，";
			}else{
				head+=DateUtil.getYearFirst(year)+"至"+DateUtil.getYearLast(year)+"，";
			}
			if(null!=cbbmName&&!cbbmName.equals("")){
				html=" <p>"+head+cbbmName+"核发各类规划审批证书共<u><b> "+totalCount+" </b></u>件，其中";
			}else{
				html=" <p>"+head+"青岛市共核发各类规划审批证书共<u><b> "+totalCount+" </b></u>件，其中";
			}
	...
	public Object StaticCountByYear() throws Exception{
		...
		String cbbmId=pd.getString("cbbmId");
		String cbbmName=pd.getString("cbbmName");
		String cbbmNameShow="青岛市";
		if(null!=cbbmName&&!cbbmName.equals("")){
			cbbmNameShow=cbbmName;
		}
		int year=Integer.parseInt(pd.getString("year"));
		List<Dictionaries> fwlxList = new ArrayList<Dictionaries>();
		List<String> fwlxNameList=new ArrayList<String>();
		...
		int currentYear=Integer.parseInt(DateUtil.getYear());
		String currentEndDate=DateUtil.getYearLast(year);
		String lastEndDate=DateUtil.getYearLast(year-1);
		if(year==currentYear){
			 currentEndDate=DateUtil.getDay();
			 lastEndDate=DateUtil.getDataByIntervalYear(1);
		}
		try {
			pd.put("BIANMA", "021");
			fwlxList = dictionariesService.listAllItemsByCodeValue(pd);
			...
			MongoCollection<Document> collection = mongoClient.getDatabase(MongoDBUtil.getDbName()).getCollection("fawen");
			Document document=new Document();
			document.append("FZRQ", new Document("$gte",DateUtil.getYearFirst(year)).append("$lte", currentEndDate));
			if(null!=cbbmId&&!cbbmId.equals("")){
				document.append("REGION_CODE", cbbmId);
			}
			document.append("ZT", "1");
			...
			document1.append("FZRQ", new Document("$gte",DateUtil.getYearFirst(year-1)).append("$lte", lastEndDate));
			if(null!=cbbmId&&!cbbmId.equals("")){
				document1.append("REGION_CODE", cbbmId);
			}
			...
			if(riseCountList.get(0)>=0){
				html=" <p>与"+(year-1)+"年同期相比，"+year+"年"+cbbmNameShow+"核发各类规划审批证书增加<u><b> "+riseCountList.get(0)+" </b></u>件，其中";
			}else{
				html=" <p>与"+(year-1)+"年同期相比，"+year+"年"+cbbmNameShow+"核发各类规划审批证书减少<u><b> "+Math.abs(riseCountList.get(0))+" </b></u>件，其中";
			}
			for(int i=1;i<fwlxNameList.size();i++){
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value="/StaticMonthCountByYear")
	@ResponseBody
	public Object StaticMonthCountByYear() throws Exception{
		Map<String,Object> map = new HashMap<String,Object>();
		String code = "success"; 
		PageData pd = new PageData();
		pd=this.getPageData();
		...
		try {
			...
			for(int i=0;i<fwlxList.size();i++){
				...
				for(int j=12;j>0;j--){
					if(i==0){
						monthList.add(DateUtil.formatMonthOfYear(year, j));
					}
					Document document=new Document();
					document.append("FZRQ", new Document("$gte",DateUtil.getBeginDataOfMonthAndYear(year, j)).append("$lte", DateUtil.getEndDataOfMonthAndYear(year, j)));
					if(null!=cbbmId&&!cbbmId.equals("")){
						document.append("REGION_CODE", cbbmId);
					}
					document.append("ZT", "1");
			...
			String html="";
			html=" <p>"+year+"年各月（"+monthList.get(monthList.size()-1)+"-"+monthList.get(0)+"）"+cbbmNameShow+"核发情况为：";
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value="/compareMonthCountByYear")
	@ResponseBody
	public Object compareMonthCountByYear() throws Exception{
		Map<String,Object> map = new HashMap<String,Object>();
		String code = "success"; 
		PageData pd = new PageData();
		pd=this.getPageData();
		...
		try {
			...
			for(int j=1;j<=12;j++){
				...
				Document document1=new Document();
				document1.append("FZRQ", new Document("$gte",DateUtil.getBeginDataOfMonthAndYear(year-1, j)).append("$lte", DateUtil.getEndDataOfMonthAndYear(year-1, j)));
				if(null!=cbbmId&&!cbbmId.equals("")){
					document1.append("REGION_CODE", cbbmId);
				}
				document1.append("ZT", "1");
				...
				String html="";
				html+="<p>"+year+"年与"+(year-1)+"年相比，"+cbbmNameShow+"核发变化情况：";
				for(int i=0;i<monthList.size();i++){
	...
	public Object StaticCountByHalfYear() throws Exception{
		...
		try {
			...
			for(int i=0;i<fwlxList.size();i++){
			 ...
			 //<p>近6个月（2018年1月-2018年6月）核发情况为：1月总数<b> 12 </b>件，2月总数<b> 5 </b>件，3月总数<b> 10 </b>件，4月总数<b> 15 </b>件，5月总数<b> 8 </b>件，6月总数<b> 10 </b>件。</p>
			String html="";
			html=" <p>近6个月（"+monthList.get(0)+"-"+monthList.get(monthList.size()-1)+"）"+cbbmNameShow+"核发情况为： ";
			for(int i=0;i<monthList.size();i++){
				Long count=0l;
				for(int j=0;j<fwlxCountList.size();j++){
					List<Long> list = (List<Long>) fwlxCountList.get(j);
	...
	public Object compareCountByHalfYear() throws Exception{
		...
		try {
			...
			for(int j=5;j>=0;j--){
				...
				//<p>2017年1月-6月 与 2018年1月-6月 相比，核发变化情况：1月增加<b> 3 </b>件，2月减少<b> 1 </b>件，3月增加<b> 0 </b>件，4月增加<b> 5 </b>件，5月增加<b> 0 </b>件，6月增加<b> 2 </b>件。</p>
				String html="";
				html+="<p>"+DateUtil.getMonthOfYearByInterval(17)+"-"+DateUtil.getMonthOfYearByInterval(12)+"与"+DateUtil.getMonthOfYearByInterval(5)+"-"+DateUtil.getMonthOfYearByInterval(0)+"相比，"+cbbmNameShow+"核发变化情况：";
				for(int i=0;i<monthList.size();i++){
					Long gap=fwlxHalfThisYearCountList.get(i)-fwlxHalfLastYearCountList.get(i);
					if(gap>=0){
						if(i==0){
