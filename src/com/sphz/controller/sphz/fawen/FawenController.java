...
public class FawenController extends BaseController {
	...
	public Map<String,Object> getListByZT(String zt,PageData pd){
		...
		try{
			...
			String iSortCol = pd.getString("iSortCol_0");//排序的列号
            String sSortDir = pd.getString("sSortDir_0");//排序的顺序asc or desc
            String sortColumn = pd.getString("mDataProp_"+iSortCol);//排序的列。
            if(sortColumn.equals("SQRQ")){
			...
			document.append(sortColumn, sort);
			//document.append("LRSJ", -1);
			/*if(zt.equals("0")){
				document.append("SUBMIT_TIME", -1);
			}else{
				//document.append("FZRQ", -1);
				document.append(sortColumn, sort);
			}*/
	...
	public ModelAndView goAddInfo() throws Exception{
		...
		PageData dictionary = dictionariesService.findByBianma(pd);
		if(null!=dictionary){
			pd.put("ORG_NAME",dictionary.getString("NAME"));
		}
		mv.addObject("pd",pd);
	...
	public Object edit(HttpServletRequest request) throws Exception{
		...
		try {
			...
			FindIterable<Document> doc=mongoClient.getDatabase(MongoDBUtil.getDbName()).getCollection("xiangmu").find(new Document("XMWYM",xmwym));
			List<Map<String, Object>> projList = MongoDBUtil.documents2MapList(doc);
			//从后往前录入审批阶段问题处理
			//String spjdArray[]={"","条","地","建","核"};
			
			List<String>spjdList=new ArrayList<String>();
			spjdList.add("");
			spjdList.add("条");
			spjdList.add("地");
			spjdList.add("建");
			...
			if(projList!=null&&projList.size()>0){
				spjd=MongoDBUtil.convertObject2String(projList.get(0).get("SPJD"));
			}
			//int originIndex = Arrays.binarySearch(spjdArray, spjd);
			int originIndex = spjdList.indexOf(spjd);
			
			if(fwlxId.indexOf("选址、条件")>-1){
				int newIndex = spjdList.indexOf("条");
				if(newIndex>originIndex){
					spjd="条";
				}
			}else if(fwlxId.indexOf("用地")>-1){
				int newIndex = spjdList.indexOf("地");
				if(newIndex>originIndex){
					spjd="地";
				}
				
			}else if(fwlxId.indexOf("建设许可")>-1){
				int newIndex = spjdList.indexOf("建");
				if(newIndex>originIndex){
					spjd="建";
				}
				
			}else if(fwlxId.indexOf("竣工")>-1){
				int newIndex = spjdList.indexOf("核");
				if(newIndex>originIndex){
					spjd="核";
				}
			}
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value="/exportXMXX")
	public ModelAndView exportXMXX(HttpServletRequest request) throws Exception{
		ModelAndView mv = this.getModelAndView();
		ExportManager  exportManager=(ExportManager)Utils.getApplicationContext().getBean(ExportManager.BEAN_ID);
		Map<String,Object> parameters=new HashMap<String,Object>();
		/*String p=request.getParameter("param");
		parameters.put("param", request.getParameter("param"));*/
		HtmlReport htmlReport = exportManager.exportHtml("file:项目信息表全.ureport.xml",request.getContextPath(),parameters);
		String html="<style type=\"text/css\">";
		html+=htmlReport.getStyle();
		html+="</style>";
		//request.getParameter("param");
		html+=htmlReport.getContent();
		mv.addObject("html", html);
		mv.setViewName("sphz/fawen/xmxx");
		return mv;
	}
