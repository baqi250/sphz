package com.sphz.controller.system.login;
...
import com.sphz.service.fhoa.datajur.DatajurManager;
import com.sphz.service.fhoa.department.DepartmentManager;
import com.sphz.service.system.appuser.AppuserManager;
import com.sphz.service.system.buttonrights.ButtonrightsManager;
import com.sphz.service.system.dictionaries.DictionariesManager;
import com.sphz.service.system.fhbutton.FhbuttonManager;
import com.sphz.service.system.fhlog.FHlogManager;
import com.sphz.service.system.loginimg.LogInImgManager;
import com.sphz.service.system.menu.MenuManager;
...
import com.sphz.util.RightsHelper;
...
//				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						...
						user.setCNM(pd.getString("NUMBER"));
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				session.setAttribute(Const.SESSION_NUMBER, user.getCNM());					//放入用户编号到session
				...
				List<Menu> sphzMenuList = new ArrayList<Menu>();
				for(int i=0;i<menuList.size();i++){
					if(menuList.get(i).getMENU_NAME().equals("规划审批汇总管理")){
						//sphzMenuList.add(allmenuList.get(i)); 
						sphzMenuList=menuList.get(i).getSubMenu(); 
					}
				}
				...
				mv.addObject("sphzMenuList", sphzMenuList);
			}else {
...
/*	*//**
	...
	public ModelAndView defaultPage1() throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd.put("userCount", Integer.parseInt(userService.getUserCount("").get("userCount").toString())-1);				//系统用户数
		pd.put("appUserCount", Integer.parseInt(appuserService.getAppUserCount("").get("appUserCount").toString()));	//会员数
