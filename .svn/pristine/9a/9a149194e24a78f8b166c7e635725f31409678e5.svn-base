...
<body class="no-skin">
	...
	<div class="main-content">
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
								<form action="notice/${msg }.do" name="noticeForm" id="noticeForm" method="post" enctype="multipart/form-data">
									<input type="hidden" name="notice_id" id="notice_id" value="${pd.notice_id }"/>
									<input type="hidden" name="picture_url" id="picture_url" value="${pd.picture_url }"/>
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										<tr>
											...
											name="notice_forum" id="notice_forum" data-placeholder="请选择信息板块" onchange="change(this.value)" 
											style="vertical-align:top;width:100%;">
												<option value=""></option>
												<c:forEach items="${notice_forumList}" var="item">
													<option value="${item.DICTIONARIES_ID }"
													<c:if test="${item.DICTIONARIES_ID == pd.notice_forum }">selected</c:if>>${item.NAME }</option>
												</c:forEach>
											</select></td>
										</tr>
										...
										<tr id="wjtr" style="display:none;">
											<td style="width:90px;text-align: right;padding-top: 13px;">新闻图片:</td>
											<td><input type="file" name="file" id="file"  accept="image/png, image/jpeg"  style="width:100%;"/></td>
										</tr>
	...
	if("${msg }"=="edit"){
		var str='';
			...
			str+='<table id="file-table" class="table table-striped table-bordered table-hover"  style="margin-top:0px;">'+
					'<tr>'+
						...
						'<td class="center" >文件路径</td>'+
						'<td class="center" >发布时间</td>'+
						'<td class="center">操作</td>'+
					'</tr>'+
				'</table>'+
			'</td></tr>';
			$("#uploadPannel").before(str);
		}
		
		if("${msg }"=="edit"){
			$.ajax({
					type: "POST",
					url: '<%=basePath%>notice/listFileByNoiceId.do?tm='+new Date().getTime(),
			    	data: {'':'1',notice_id:$("#notice_id").val()},
					dataType:'json',
					cache: false,
					success: function(data){
						...
						for(var i=0;i<fileList.length;i++){
							...
							row +="<td class='center'>"+(i+1)+"</td>";
							row +="<td class='center'>"+item.name+"</td>";
							row +="<td class='center'>"+ item.path+"</td>"; 
							row +="<td class='center'>"+ item.create_time+"</td>";
							row +="<td class='center' style='width:130px;'>";
							/* row +="<a style='cursor:pointer;' class='green' onclick='download("+item.file_id+");' title='下载'>";
							row +="<i class='ace-icon fa fa-pencil bigger-130'></i></a>&nbsp;"; */
		...
		function del(id){
			if(confirm("确定要删除?")){ 
				var url = "<%=basePath%>notice/deleteFile.do?file_id="+id+"&tm="+new Date().getTime();
		...
		console.log("${msg }");
		console.log("${pd.notice_forum }");
		if("${msg }"=="edit"){
	
			$("#notice_forum").attr("disabled","true");
			
			if("${pd.notice_forum }"=="4317964c137741a8800d23a960d86854"){
				$("#wjtr").css("display","");
			}else{
		...
		function change(value){
			if(value=="4317964c137741a8800d23a960d86854"){
				$("#wjtr").css("display","");
			}else{
				$("#wjtr").css("display","none");
			}
