...
define([
	...
	var polygon2;
	var myfeatures = [];
	var count=0;//代表layer中包括的layerinfos的长度
	var urls = [];//用来保存服务的url，/0  /1.。。。
	function showResults(results){
		//通过遍历，将两层features加入到myfeatures数组中
		for(var i=0;i<count;i++){
			myfeatures[i]= results.features;
		}
	...
	var h=declare("MapTree",null,{
		...
		//gp服务中，使用queryTask获取feature
		getFeatures:function(){
			var query = new Query();
			var queryTask = [];
			/*for(var i=0;i<count;i++){
				queryTask[i] = new QueryTask(urls[i]);
			}*/  //周一回来从此处改
			console.log(urls);
		    ...
		    query.where = "1=1";
		    /*for(var i=0;i<count;i++){
		    	queryTask[i].execute(query,showResults);
			}*///配合上面的task[i]
    		queryTask.execute(query,showResults);
		},
		//获取gp服务中选中layer的url
		getLayerUrl:function(){
			var layer = mapApp.map.getLayer("zhjtgh");
			//console.log(layer);
			var len = layer.layerInfos.length;
			count = len;
			for(var i=0;i<len;i++){
				urls[i] = "http://192.168.109.44:6080/arcgis/rest/services/qxxq/kg_yd/MapServer/"+i
			}
		},
		/**
         * 初始化透明度滑块
		...
		init: function(){//初始化ztree
			...
			$.get("../map/maplist",function (rc){
				...
				polygon2=new FeatureLayer("http://192.168.109.44:6080/arcgis/rest/services/qxxq/kg_yd/MapServer/0",{
			...
			$("#uploadcad").click(function(){
				//alert(1);
					$("#cadForm").ajaxSubmit(function(data){
						...
						var graresult=new esri.layers.GraphicsLayer();
						var map = mapApp.map;
