        ...
        opts.templateResult = function(data, container) {
            ...
            if (data.element) {
                ...
                if ($(container).hasClass("non-leaf")) {
                    var $iteme = $("<span class='item-label' class='expand-collapse' onmouseup='expColMouseupHandler(event);'></span>").append(label);
    ...
    function buildSelect(treeData, $el) { // build Select options according to Select-to-Tree specification
        function buildOptions(dataArr, curLevel, pup) {
            for (var i = 0; i < dataArr.length; i++) {
                ...
                $opt.val(data[treeData.valFld || "dictionaries_ID"]);
                if ($opt.val() === "") {
                ...
                $el.append($opt);
                var inc = data[treeData.incFld || "subDic"];
